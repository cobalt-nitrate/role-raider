"""Role Raider configuration: paths, platform detection, user data."""
...
APP_DIR = Path(os.environ.get("ROLE_RAIDER_DIR", Path.home() / ".role_raider"))

# Core paths
DB_PATH = APP_DIR / "role_raider.db"
...
def load_profile() -> dict:
    """Load user profile from ~/.role_raider/profile.json."""
    import json
    if not PROFILE_PATH.exists():
        raise FileNotFoundError(
            f"Profile not found at {PROFILE_PATH}. Run `role_raider init` first."
        )
    return json.loads(PROFILE_PATH.read_text(encoding="utf-8"))


def load_search_config() -> dict:
    """Load search configuration from ~/.role_raider/searches.yaml."""
    import yaml
...
def load_env():
    """Load environment variables from ~/.role_raider/.env if it exists."""
...
def check_tier(required: int, feature: str) -> None:
    ...
    if required >= 2 and not any(os.environ.get(k) for k in ("GEMINI_API_KEY", "OPENAI_API_KEY", "LLM_URL")):
        missing.append("LLM API key — run [bold]role_raider init[/bold] or set GEMINI_API_KEY")
    if required >= 3:
