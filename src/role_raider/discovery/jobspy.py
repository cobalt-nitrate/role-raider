...
parses salary ranges, and stores everything in the Role Raider database.
...
from role_raider.database import get_connection, init_db, store_jobs
...
def run_discovery(cfg: dict | None = None) -> dict:
    ...
    if not cfg:
        log.warning("No search configuration found. Run `role_raider init` to create one.")
