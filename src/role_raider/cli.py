"""Role Raider CLI — the main entry point."""
...
def _version_callback(value: bool) -> None:
    if value:
        console.print(f"[bold]role_raider[/bold] {__version__}")
        raise typer.Exit()
...
def main(
    ...
    """Role Raider — AI-powered end-to-end job application pipeline."""


@app.command()
def init() -> None:
    """Run the first-time setup wizard (profile, resume, search config)."""
    from role_raider.wizard.init import run_wizard
...
def apply(
    ...
    from role_raider.config import check_tier, PROFILE_PATH as _profile_path
    from role_raider.database import get_connection

    # --- Utility modes (no Chrome/Claude needed) ---

    if mark_applied:
        from role_raider.apply.launcher import mark_job
    ...
    if reset_failed:
        from role_raider.apply.launcher import reset_failed as do_reset
        count = do_reset()
    ...
    if not _profile_path.exists():
        console.print(
            ...
            "Run [bold]role_raider init[/bold] to create your profile first."
        )
        raise typer.Exit(code=1)
    ...
    if not (gen and url):
        ...
        if ready == 0:
            console.print(
                ...
                "Run [bold]role_raider run score tailor[/bold] first to prepare applications."
            )
            raise typer.Exit(code=1)

    if gen:
        from role_raider.apply.launcher import gen_prompt, BASE_CDP_PORT
        target = url or ""
        if not target:
    ...
    from role_raider.apply.launcher import main as apply_main
...
def status() -> None:
    ...
    from role_raider.database import get_stats

    stats = get_stats()

    console.print("\n[bold]Role Raider Pipeline Status[/bold]\n")
...
def dashboard() -> None:
    ...
    from role_raider.view import open_dashboard
...
def doctor() -> None:
    ...
    if PROFILE_PATH.exists():
        ...
        results.append(("profile.json", fail_mark, "Run 'role_raider init' to create"))
    ...
    if RESUME_PATH.exists():
        ...
        results.append(("resume.txt", fail_mark, "Run 'role_raider init' to add your resume"))

    # Search config
    if SEARCH_CONFIG_PATH.exists():
        results.append(("searches.yaml", ok_mark, str(SEARCH_CONFIG_PATH)))
    else:
        results.append(("searches.yaml", warn_mark, "Will use example config — run 'role_raider init'"))
    ...
    if has_gemini:
        ...
        results.append(("LLM API key", fail_mark,
                        "Set GEMINI_API_KEY in ~/.role_raider/.env (run 'role_raider init')"))
    ...
    # --- Render results ---
    console.print()
    console.print("[bold]Role Raider Doctor[/bold]\n")
    ...
    # Tier summary
    from role_raider.config import get_tier, TIER_LABELS
    tier = get_tier()
